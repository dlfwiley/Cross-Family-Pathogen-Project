...
MODEL 1 REDUCED MODEL SET (WITHOUT LONGITUDE)
...
MODEL 2 External/combination tissues only:
...
#------------------------------------------------------ . \# STEP 4: Compare model output Picking best model via OOB & mtry ####RF Model 0: Full
...
We will select model 1 (-longitude) for our final model. Now we can move forward with training and testing the model over 100 iterations and averaging the output.
...
There is class imbalance which will need to be fixed - as it stands now, our model is very good at predicting majority class (uninfected), but very bad at predicting our class of interest (minority). #------------------------------------------------------ . \# STEP 7: Balancing out models . ####Upsampling
...
RF_model
# Call:
#  randomForest(formula = as.factor(Status_Bd) ~ ., data = pred_samp,      ntree = 2000, importance = T, strata = as.factor(pred_samp[,          "Status_Bd"]), sampsize = c(23, 23), mtry = best.m1[1]) 
...
#Tissue type - model check Checking that our model output matches after removing internal-only tissues.
