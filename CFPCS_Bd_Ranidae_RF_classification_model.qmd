---
title: "Random forests to predict Bd infection status across frogs in the family Ranidae"
author: Dani Wiley
date:   last-modified
editor: visual
description: |
...
#Bd prevalence dataset: contains all positive and negative individuals
Bd_p_df <- filter(Bd_p_df, Bd_p_df$Fam == "Ranidae") # 789 observations
...
Bd_p_df.exto$Tissue <- as.factor(Bd_p_df.exto$Tissue) # 353
Bd_p_df.inte$Tissue <- as.factor(Bd_p_df.inte$Tissue) # 144
...
```{r}
############################### RANDOM FORESTS ###############################
################# 0 - BD STATUS - FULL MODEL - 4 species #################
#############################################################################
#MODEL SET 0 (FULL)
set.seed(91011)
...
RFfull_MDA.p  <- ggplot(MDA, 
  ...
  geom_point(shape = 21, colour = "black", fill = "#0066CC", size = 4, stroke = 1) + 
...
```{r}
############################### RANDOM FORESTS ###############################
################# 1 - BD STATUS - NO LONGITUDE - 4 species #################
#############################################################################
...
m1.1 <- vip(rf1_Bdp, geom = "point",
          ...
          horizontal = TRUE,
          include_type = TRUE)
BdpRF1 <- m1.1 + geom_point(shape = 21, colour = "black", fill = "#0066CC", size = 4, stroke = 1) + ggtitle("Random Forest classification - Model 1 Bd infection status")
BdpRF1
...
m_bdp <- vip(rf1_Bdp, geom = "point",
          ...
          horizontal = TRUE,
          include_type = TRUE)
m_bdp <- m_bdp + geom_point(shape = 21, colour = "black", fill = "#0066CC", size = 4, stroke = 1) + ggtitle("Random Forest classification - Trained Model Set for Bd infection status")
m_bdp
...
Bd0=RF_Bdp.1[RF_Bdp.1$Status_Bd=="0",] #344
Bd1=RF_Bdp.1[RF_Bdp.1$Status_Bd=="1",] #161
...
#declare training datasets for both classes
Bd0=RF_Bdp.1[RF_Bdp.1$Status_Bd=="0",] #344 cases
Bd1=RF_Bdp.1[RF_Bdp.1$Status_Bd=="1",] #161 cases
...
for (i in 1:100) {
  ...
  RF_model <- randomForest(as.factor(Status_Bd)~.
                  ...
                  , sampsize = c(161, 161) #subsample equally, so to build a model without majority class bias
                  , mtry = best.m1[1])
...
RF_model
# Call:
#  randomForest(formula = as.factor(Status_Bd) ~ ., data = pred_samp,      ntree = 2000, importance = T, strata = as.factor(pred_samp[,          "Status_Bd"]), sampsize = c(161, 161), mtry = best.m1[1]) 
...
for (i in 1:100) {
  ...
  RF_model <- randomForest(as.factor(Status_Bd)~.
                  ...
                  , importance=T
                  , strata = as.factor(pred_samp[,"Status_Bd"])
                  , sampsize = c(110, 110) #subsample at similar rates to what we find in nature
