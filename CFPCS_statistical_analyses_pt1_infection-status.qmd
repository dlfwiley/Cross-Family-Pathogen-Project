...
ag_df_p_bd_exto <- subset(ag_df_p_bd,
                          ag_df_p_bd$Bd_Tissue == "Combination" |
                          ag_df_p_bd$Bd_Tissue == "External_tissues"  )

age.Bd.table_exto <- table(ag_df_p_bd_exto$Age, ag_df_p_bd_exto$Status_Bd)
...
corrplot(Rv.tiss.chi$residuals, 
         ...
         is.cor = FALSE)
```
We see that there is a clear difference in infection prevalence between internal only and exto (combination/external tissues) where internal only is not detecting Bd in our samples. 
Next, we need to understand how this is distributed in our sampling to potentially connect areas of bias in our analyses and models.
```{r}
...
Bd is not expected to be detectable in liver/internal only tissues, as it primarily infects the skin. 
We see that internal tissues are potentially interacting with our latitude results.
There are significantly lower than expected Bd infections in our internal only tissues (see section below) *and* variable significance changes when excluding internal-only tissues (see section above).
...
#NOT SIGNIFICANT
```
Bufonidae - no longer significant.
Hylidae - still not significant.
Ranidae - no longer significant.
...
####Visualization
Distribution of latitude values by infection status.
```{r}
#Visualize dataset
...
####Visualization
Distribution of longitude values by infection status.
```{r}
#Visualize dataset
#Set colors for pathogen comparisons
Bd_c <- c("darkgrey","#40B0A6")
...
# NOT SIGNIFICANT
```
No significant difference in elevation means between Pr infected vs. uninfected frogs.
Rv:
```{r}
#TEST FOR EQUAL VARIANCES BETWEEN INFECTED VS. UNINFECTED ELEVATION VALUES.
Rv_pos <- filter(df_p_rv, df_p_rv$Status_Rv == "1")
...
####Visualization
Distribution of elevation values by infection status.
...
####Visualization
Distribution of TPC1 values by infection status.
...
```
Not considering anything else in the data, there is a significant relationship between TPC2 and Pr status where with unit increase in TPC2, the log odds of Pr positive increase by $e^{1.0496}$ or the odds of being infected with Bd decrease by a factor $2.86$.\ Low sample sizes though.
...
####Visualization
Distribution of TPC2 values by infection status.
...
BdLat_ttest.prev <-t.test(PPC1 ~ Status_Bd, data = df_p_bd, alternative = "two.sided", var.equal = TRUE)
BdLat_ttest.prev
# 	Two Sample t-test
# 
...
####Visualization
Distribution of PPC1 values by infection status.
...
BdLat_ttest.prev <-t.test(PPC2 ~ Status_Bd, data = df_p_bd, alternative = "two.sided", var.equal = FALSE)
BdLat_ttest.prev
...
#Infection status figures for manuscript
Figure 4a - TPC1, TPC2
