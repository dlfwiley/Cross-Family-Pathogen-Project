---
title: "Random forests to predict Bd infection status across frogs in the family Hylidae"
author: Dani Wiley
date:   last-modified
editor: visual
description: |
...
#Bd prevalence dataset: contains all positive and negative individuals
Bd_p_df <- filter(Bd_p_df, Bd_p_df$Fam == "Hylidae") # 789 observations
...
Bd_p_df.exto$Tissue <- as.factor(Bd_p_df.exto$Tissue) # 241
Bd_p_df.inte$Tissue <- as.factor(Bd_p_df.inte$Tissue) # 167
...
RFfull_MDA.p  <- ggplot(MDA, 
  ...
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) + 
...
m1.1 <- vip(rf1_Bdp, geom = "point",
          ...
          horizontal = TRUE,
          include_type = TRUE)
BdpRF1 <- m1.1 + geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) + ggtitle("Random Forest classification - Model 1 Bd infection status")
BdpRF1
...
m_bdp <- vip(rf1_Bdp, geom = "point",
          ...
          horizontal = TRUE,
          include_type = TRUE)
m_bdp <- m_bdp + geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) + ggtitle("Random Forest classification - Trained Model Set for Bd infection status")
m_bdp
...
#Now we can visualize MDA for each variable
par(mfrow=c(1,2))
Uninfected_MDA <- ggplot(mda_means_Bd0_u, aes(x=RF_varimp_u.var, y = RF_varimp_u.X0)) +
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
Infected_MDA <- ggplot(mda_means_Bd1_u, aes(x=RF_varimp_u.var, y = RF_varimp_u.X1)) +
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
```
...
RF_Bpd_GINI_100_u
#write.csv(gini_means, file="RF_HYLI_model_upsampling_importance_gini_nolongitude_20240526.csv", row.names = T)
#write.csv(mda_means, file="RF_HYLI_model_upsampling_importance_mda_nolongitude_20240526.csv", row.names = T)
RF_model_u
```
...
#declare training datasets for both classes
Bd0=RF_Bdp.1[RF_Bdp.1$Status_Bd=="0",] #424cases
Bd1=RF_Bdp.1[RF_Bdp.1$Status_Bd=="1",] #32 cases
...
for (i in 1:100) {
  ...
  RF_model <- randomForest(as.factor(Status_Bd)~.
                  ...
                  , sampsize = c(32, 32) #subsample equally, so to build a model without majority class bias
                  , mtry = best.m1[1])
...
#output these predictions to a file
#write.csv(StatusPredicted, file="RF_HYLI_downsampling_model_misclassified.csv", row.names=FALSE)
...
#Now we can visualize MDA for each variable
par(mfrow=c(1,2))
Uninfected_MDA <- ggplot(mda_means_Bd0, aes(x=RF_varimp.var, y = RF_varimp.X0)) +
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
Infected_MDA <- ggplot(mda_means_Bd1, aes(x=RF_varimp.var, y = RF_varimp.X1)) +
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
```
...
STEP 8: Validation of final model We are choosing Model set 1 (-longitude).
...
#declare training datasets for both classes
Bd0=RF_Bdp.1[RF_Bdp.1$Status_Bd=="0",] # 424
Bd1=RF_Bdp.1[RF_Bdp.1$Status_Bd=="1",] # 32
...
#output these predictions to a file
#write.csv(StatusPredicted, file="BdStatus_RF_HYLI_FINAL_upsampling_model_misclassified_20240526.csv", row.names=FALSE)
...
colMeans(RF_model_error,na.rm=T)
#       OOB         0         1 
# 0.3207813 0.3231250 0.3184375 
#write.csv(RF_model_error,file = "BdStatus_RF_HYLI_FINAL_model_upsampling_error_nolongitude_20240526.csv",row.names=F)
...
RF_Bpd_MDA_100 <-
  ...
  theme(axis.text.x = element_text(angle=60, hjust=1,size=10)) + 
  theme(axis.text.y = element_text(size=10)) + 
  ggtitle("Bd status Random Forest - Hylidae") +
  theme(axis.title = element_text(size=12,face="bold")) + 
  xlab("") + ylab("")
RF_Bpd_MDA_100
...
Uninfected_MDA <-ggplot(
  ...
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1)) + 
  ggtitle("Bd status Random Forest - Uninfected - Hylidae") +
  theme(axis.title = element_text(size=12,face="bold")) + 
  xlab("") + 
  ylab("")
Infected_MDA <- ggplot(
  ...
  geom_point(shape = 21, colour = "black", fill = "#009900", size = 4, stroke = 1) +
  theme(axis.text.x = element_text(angle=60, hjust=1)) +
  ggtitle("Bd status Random Forest - Infected - Hylidae") +
  theme(axis.title = element_text(size=12,face="bold")) + 
  xlab("") + 
  ylab("")
```
...
#declare training datasets for both classes
Bd0=RF_Bdp.1.exto[RF_Bdp.1.exto$Status_Bd=="0",] #216
Bd1=RF_Bdp.1.exto[RF_Bdp.1.exto$Status_Bd=="1",] #25
...
for (i in 1:100) {
  ...
  RF_model <- randomForest(as.factor(Status_Bd)~.
                  , data=pred_samp, ntree=2000, importance=T
                  , strata = as.factor(pred_samp[,"Status_Bd"])
                  , sampsize = c(25, 25) #subsample equally, so to build a model without majority class bias
                  , mtry = best.m2[1] 
                  )
